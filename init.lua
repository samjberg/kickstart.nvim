...
require('lazy').setup({
  ...
  { -- Highlight, edit, and navigate code
    ...
    lazy = false,
    build = function() require('nvim-treesitter').update():wait(300000) end,
    config = function()
      ...
      local ensure_installed = { 'bash', 'c', 'cpp', 'diff', 'html', 'lua', 'luadoc', 'markdown', 'markdown_inline', 'python', 'query', 'vim', 'vimdoc' }
      local installed = {}
      ...
      for _, lang in ipairs(ensure_installed) do
        local query_path = vim.fs.joinpath(vim.fn.stdpath 'data', 'site', 'queries', lang, 'highlights.scm')
        if not installed[lang] or not vim.uv.fs_stat(query_path) then needs_install[#needs_install + 1] = lang end
      end
      if #needs_install > 0 then treesitter.install(needs_install, { force = true, summary = true }) end
      ...
      vim.api.nvim_create_autocmd({ 'FileType', 'BufEnter', 'BufWinEnter' }, {
        ...
        callback = function(args) start_highlight(args.buf) end,
      })
      ...
      require('custom.treesitter_cpp_motions').setup()
    end,
  },
